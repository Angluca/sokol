...
#   gen_nature.py
#
#   Generate nature bindings.
#-------------------------------------------------------------------------------
import gen_ir
import gen_util as util
import os, shutil, sys

bindings_root = 'sokol-nature'
...
overrides = {
    # `any` is treated specially in nature.
    'any': 'any_',
    'type': 'type_',
    'ptr': 'ptr_',
...
aliases = {
    ...
    "void (*)(void *)":
        ("data_cb_t", "fn(anyptr)"),
    "void *(*)(size_t, void *)":
        ("alloc_cb_t", "fn(uint, anyptr): anyptr"),
    "void (*)(void *, void *)":
        ("free_cb_t", "fn(anyptr, anyptr)"),
    "void (*)(const char *, uint32_t, uint32_t, const char *, uint32_t, const char *, void *)":
        ("log_cb_t", "fn(rawptr<u8>, u32, u32, rawptr<u8>, u32, rawptr<u8>, anyptr)"),
    "void (*)(void)":
        ("cb_t", "fn()"),
    "void (*)(const sapp_event *)":
        ("event_cb_t", "fn(rawptr<event_t>)"),
    "void (*)(const sapp_event *, void *)":
        ("event_data_cb_t", "fn(rawptr<event_t>, anyptr)"),
    "void (*)(const sapp_html5_fetch_response *)":
        ("response_cb_t", "fn(rawptr<html5_fetch_response_t>)"),
    "void (*)(float *, int, int)":
        ("stream_cb_t", "fn(rawptr<f32>, int, int)"),
    "void (*)(float *, int, int, void *)":
        ("stream_data_cb_t", "fn(rawptr<f32>, int, int, anyptr)"),
}
...
def get_nature_module_path(c_prefix):
    return f'{module_root}'

def get_csource_path(c_prefix):
    return f'{c_root}/{c_source_names[c_prefix]}'

def make_nature_module_directory(c_prefix):
    path = get_nature_module_path(c_prefix)
...
def map_type(type, prefix, sub_type):
    ...
    if type == "void":
        ...
        return as_struct_type(type, prefix)
        ...
        return f"rawptr<{as_struct_type(util.extract_ptr_type(type), prefix)}>"
    elif is_prim_ptr(type):
        return f"rawptr<{as_prim_type(util.extract_ptr_type(type))}>"
...
def gen_c_imports(inp, c_prefix, prefix):
    ...
    for decl in inp['decls']:
        if decl['kind'] == 'func' and not decl['is_dep'] and not check_ignore(decl['name']):
            ...
            l(f'#linkid {decl["name"]}')
            tail_type = '' 
            if  res_type != '': 
                tail_type = ': ' + res_type
            l(f'fn {check_override(util.as_lower_snake_case(decl["name"], c_prefix))}({args}){tail_type}')
            # if res_type == '':
                # l(f'fn {check_override(util.as_lower_snake_case(decl["name"], c_prefix))}({args})')
            # else:
                # l(f'fn {check_override(util.as_lower_snake_case(decl["name"], c_prefix))}({args}): {res_type}')
    l('')
...
def gen_consts(decl, prefix):
    for item in decl["items"]:
        ...
        l(f"const {as_upper_snake_case(item_name, prefix)} = {item['value']}")
    l('')

def gen_struct(decl, prefix):
    c_struct_name = check_override(decl['name'])
    struct_name = as_struct_type(c_struct_name, prefix)
    l(f'type {struct_name} = struct' + ' {')
...
def gen_enum(decl, prefix):
    ...
    enum_name = as_struct_or_enum_type(enum_name, prefix).upper()
    value = "-1"
    items = decl['items']
    ...
    for item in items:
        ...
        if item_name != 'FORCE_U32':
            ...
            l(f"const {enum_name}_{item_name} = {value}")
    # l('}')
    l('')
...
def gen_module(inp, c_prefix, dep_prefixes):
    ...
    # l(f"import {module_names[c_prefix]}")

    mod = module_names[c_prefix]
    if mod == "sokol_glue":
        l(f"import sokol_gfx as {module_as_name['sokol_gfx']}")
...
def prepare():
    print('=== Generating nature bindings:')
...
def gen(c_header_path, c_prefix, dep_c_prefixes):
    if not c_prefix in module_names:
        ...
        return
    reset_globals()
    make_nature_module_directory(c_prefix)
    ...
    with open(f"{module_root}/{ir['module']}.n", 'w', newline='\n') as f_outp:
